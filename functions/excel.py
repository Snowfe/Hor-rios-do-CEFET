...
def saveTeacher(name, subject, type, classes, grade, subGroup, bimestral, prefers, limits):
    ...
    if not err:
        df = pd.read_excel('Planilha dos horários.xlsx', 'Sheet1')
        ...
        newDf.to_excel('Planilha dos horários.xlsx', index=False)
...
def saveRoom(name, local, limits):
    ...
    if not err:
        ...
        newDf.to_excel('Planilha dos horários sala.xlsx', index=False)
